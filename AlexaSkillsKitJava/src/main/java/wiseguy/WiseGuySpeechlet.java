...
public class WiseGuySpeechlet implements Speechlet {
    ...
    private SpeechletResponse handleTellMeAJokeIntent(final Session session) {
        ...
        speechOutput = "Hey Sammy how are things";
    ...
    private SpeechletResponse handleWhosThereIntent(final Session session) {
        ...
        if (session.getAttributes().containsKey(SESSION_STAGE)) {
            if ((int) session.getAttribute(SESSION_STAGE) == KNOCK_KNOCK_STAGE) {
                ...
                speechOutput = "That's too bad. Do you want help from a mentor?";
    ...
    private SpeechletResponse handleSetupNameWhoIntent(final Session session) {
        ...
        if (1==1) {
            if ((int) session.getAttribute(SESSION_STAGE) == SETUP_STAGE) {
                int jokeID = (int) session.getAttribute(SESSION_JOKE_ID);
                speechOutput = "I will email your mentor now for help. Hang in there";
